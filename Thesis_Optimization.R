...
# Time-dependent timber price function (Norway Spruce)
Pf_function <- function(t) {
...
x_prime <- function(par, VMax, g, b) {
  T <- par[1]
  psi <- par[2]
  
  integrand <- function(t) {
    u <- exp(-g * (t - b))
    numerator <- g * VMax * exp(-g * (t - b)) * exp(-u) * (-g + g * u)
    denominator <- 1 - exp(-exp(g * b))
...
PVc <- function(par, VMax, Pc, alpha, r, g, b, theta) {
  ...
  integrand2 <- function(t) {
    Pc * alpha * x_prime(t, VMax, g, b) * exp(-r * t)
  }
...
df <- read_excel("/Users/lukashasler/Library/Mobile Documents/com~apple~CloudDocs/Documents/Uni Copenhagen/Jahr 2/Thesis/Data_final.xlsx",
                 sheet = "Scenario_2")                                          # Change to Working Directory with the Excel data sheet 
